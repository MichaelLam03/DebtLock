...
struct MyDebtService {
    
    static func create(owedFrom: String, amount: String, debtDescription: String, dueDate: String) {
        
        let ref = Database.database().reference().child("Mydebts").child(User.current.uid).childByAutoId()
        let debtID = ref.key
    ...
    static func save(_ debt: Debt) {
        let ref = Database.database().reference().child("Mydebts").child(User.current.uid).child(debt.id)
        ref.updateChildValues(debt.dictValue)
    }
    
    static func existingDebts(for user: User, completion: @escaping ([Debt]) -> Void) {
        let ref = Database.database().reference().child("Mydebts").child(user.uid)
